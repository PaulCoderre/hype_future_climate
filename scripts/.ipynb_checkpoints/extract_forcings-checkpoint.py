...
results_directory= './official_forcings/'
runs_per_script = 1
file_pattern = "*.nc"
...
for directory in directory_subset:
    ...
    if os.path.isdir(directory_path):
        ...
        precipitation_mm_valid.to_csv(f'{folder_name}_Pobs.txt', sep='\t')
        tmax_celsius_valid.to_csv(f'{folder_name}_TMAXobs.txt', sep='\t')
        tmin_celsius_valid.to_csv(f'{folder_name}_TMINobs.txt', sep='\t')
        tobs_celsius_valid.to_csv(f'{folder_name}_Tobs.txt', sep='\t')
